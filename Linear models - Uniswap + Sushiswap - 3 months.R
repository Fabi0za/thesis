...
#Reading the data
data <- read.csv("df_encoded_linear.csv")
...
vars_to_test <- c("volume_usd", "imbalance", "X90D_volatility", "X90D_impermanent_loss", "total_USD_value", "return", "return_in_3months", "avg_gas")
...
#Rounding data to two digits
pdata <- pdata %>% 
  mutate_if(is.numeric, round, digits = 2)

#Formula of regression model
formula <- return_in_3months ~ volume_usd + imbalance + avg_gas + X90D_impermanent_loss + X90D_volatility + total_USD_value + return + id + pool_category_0 + pool_category_1 + Uniswap 
...
remaining_obs <- anti_join(pdata, last_six_obs, by = c("id", "start_date"))

#Creating the training set folds, starts with 175 observations and is increases by 70 observations
train_sizes <- seq(140, nrow(remaining_obs), by = 70)
...
for (i in train_sizes) {
  ...
  y_train <- train_set$return_in_3months
  ...
  y_test <- validation_set$return_in_3months
...
#Compute average metrics per folds
...
ggplot(actual_vs_predicted, aes(x = id)) +
  ...
  labs(x = "Index", y = "Return in 3 months", color = "Legend") +
  theme_minimal()
